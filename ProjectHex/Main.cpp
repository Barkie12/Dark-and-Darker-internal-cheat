...
DWORD GetProcessByName(const wchar_t* Name) {
	...
	static t_NtQuerySystemInformation f_NtQuerySystemInformation = (t_NtQuerySystemInformation)GetProcAddress(GetModuleHandleA( ("ntdll.dll")),  ("NtQuerySystemInformation"));
	ULONG buflen = 0;
	BYTE* buffer = NULL;
...
const wchar_t* dwProcessNames = L"DungeonCrawler.exe";
...
int main()
{
	...
	while (hwnd == NULL)
	{
		PrintSpace::print(("\nOpen dark and darker"));
		Sleep(50);
		system("cls");
		auto wind =  ("Dark and Darker  ");
	...
	if (!ProcessBase) 
		(MessageBoxA)(0, ("Try run .reg file."), ("ERROR 2391"), MB_OK);
	
	SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 6);
	printf(("Do not close loader !\n"));
	printf(("END KEY for Unload.\n"));
